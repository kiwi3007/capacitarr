...
func TestEmbyClient_GetWatchHistory(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		expectedPath := "/Users/user123/Items/item456"
		...
		_, _ = w.Write([]byte(`{
			"UserData": {
				"PlayCount": 3,
				"LastPlayedDate": "2024-12-25T18:30:00Z",
				"Played": true
			}
		}`))
	}))
	...
	client := NewEmbyClient(srv.URL, "test-key")
	data, err := client.GetWatchHistory("item456", "user123")
	if err != nil {
	...
	if !data.Played {
		t.Error("Expected Played to be true")
	}
	if data.LastPlayedDate.IsZero() {
		t.Error("Expected non-zero LastPlayedDate")
	}
	if data.LastPlayedDate.Year() != 2024 || data.LastPlayedDate.Month() != 12 || data.LastPlayedDate.Day() != 25 {
		t.Errorf("Unexpected LastPlayedDate: %v", data.LastPlayedDate)
	}
}

func TestEmbyClient_GetWatchHistory_Unwatched(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		_, _ = w.Write([]byte(`{
			"UserData": {
	...
	client := NewEmbyClient(srv.URL, "test-key")
	data, err := client.GetWatchHistory("item789", "user123")
	if err != nil {
		t.Fatalf("GetWatchHistory should succeed: %v", err)
	}
	if data.PlayCount != 0 {
	...
	if data.Played {
		t.Error("Expected Played to be false")
	}
	if !data.LastPlayedDate.IsZero() {
...
func TestEmbyClient_GetWatchHistory_MalformedJSON(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		w.Header().Set("Content-Type", "application/json")
	...
	client := NewEmbyClient(srv.URL, "test-key")
	_, err := client.GetWatchHistory("item456", "user123")
	if err == nil {
